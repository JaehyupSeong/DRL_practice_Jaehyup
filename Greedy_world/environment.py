...
class Env:
    ...
    def state_after_action(self, state, action_index):
        ...
        return self.check_boundary([state[0] + action[0], state[1] + action[1]])  # Return =  next_state

    @staticmethod
    # Staticmethod can be called w/o calling the instance of class, that is, it does not require "self"
    def check_boundary(state):
        state[0] = (0 if state[0] < 0 else WIDTH - 1
                    if state[0] > WIDTH - 1 else state[0])
        # if state[0] < 0 than return 0, -> Do not allow to exceed a boundary (WIDTH index: 0,1,2,3,4)
        # if state[0] > 0 and state[0] > WIDTH-1 than return WIDTH-1, -> Do not allow to exceed a boundary
        # if state[0] > 0 and state[0] < WIDTH-1 than return state[0]
        state[1] = (0 if state[1] < 0 else HEIGHT - 1
                    if state[1] > HEIGHT - 1 else state[1])
        # if state[1] < 0 than return 0, -> Do not allow to exceed a boundary (HEIGHT index: 0,1,2,3,4)
        # if state[1] > 0 and state[1] > HEIGHT-1 than return HEIGHT-1, -> Do not allow to exceed a boundary
        # if state[1] > 0 and state[1] < HEIGHT-1 than return state[1]
        return state
    ...
    def get_all_states(self):
        return self.all_state  # Return all the possible states